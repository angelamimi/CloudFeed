...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.statusCodeDictionary = [[NSMutableDictionary alloc] init];
...
- (NSURLSessionTask *)downloadWithRequest:(KTVHCDataRequest *)request delegate:(id<KTVHCDownloadDelegate>)delegate
{
    ...
    [self.statusCodeDictionary setObject:@(0) forKey:request.URL];
    task.priority = 1.0;
    [task resume];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)task didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    [self.statusCodeDictionary setObject:@(response.statusCode) forKey:task.currentRequest.URL];
    KTVHCLogDownload(@"%p, Receive response\nrequest : %@\nresponse : %@\nHTTPResponse : %@\nStatus Code : %ld\n URL : %@", self, dataRequest, dataResponse, response.allHeaderFields, (long)response.statusCode, task.currentRequest.URL);
    NSError *error = nil;
    if (!error) {
