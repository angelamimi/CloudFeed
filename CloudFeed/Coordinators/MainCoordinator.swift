...
extension MainCoordinator {
    ...
    private func initTabCoordinators(dataService: DataService) {
        ...
        guard let mediaNavController = tabBarController?.viewControllers?[0] as? UINavigationController else { return }
        guard let favoritesNavController = tabBarController?.viewControllers?[1] as? UINavigationController else { return }
        guard let settingsNavController = tabBarController?.viewControllers?[2] as? UINavigationController else { return }

        guard let mediaViewController = mediaNavController.viewControllers[0] as? MediaController else { return }
        guard let favoritesViewController = favoritesNavController.viewControllers[0] as? FavoritesController else { return }
        guard let settingsViewController = settingsNavController.viewControllers[0] as? SettingsController else { return }
    ...
    private func initSplitTabCoordinators(dataService: DataService) {
        ...
        guard let mediaNavController = tabBarController?.viewControllers?[0] as? UINavigationController else { return }
        guard let favoritesNavController = tabBarController?.viewControllers?[1] as? UINavigationController else { return }
        guard let settingsSplitController = tabBarController?.viewControllers?[2] as? UISplitViewController else { return }
        ...
        guard let menuController = settingsSplitController.viewController(for: .primary) as? MenuController else { return }
        menuController.delegate = self

        guard let mediaViewController = mediaNavController.viewControllers[0] as? MediaController else { return }
        guard let favoritesViewController = favoritesNavController.viewControllers[0] as? FavoritesController else { return }
        guard let settingsController = settingsSplitController.viewController(for: .secondary) as? SettingsController else { return }
