...
class MediaController: CollectionController {
    ...
    override func viewDidLoad() {
        ...
        viewModel.metadataVisible = compactMode == false
    ...
    override func setMetadataVisibility(_ visible: Bool) {
        ...
        for index in 0...tableView.numberOfRows(inSection: 0) {
            if let cell = tableView.cellForRow(at: IndexPath(item: index, section: 0)) as? TableCell {
                cell.setInfoVisibility(visible)
            }
        }
    ...
    private func tableCleanup() {
        for visibleCell in tableView.visibleCells {
            if let cell = visibleCell as? TableCell {
                cell.stopVideo()
            }
        }
    }
...
extension MediaController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let current = cell as? TableCell {
            current.stopVideo()
        }
    }
...
extension MediaController: Filterable {
    ...
    func filter(from: Date, to: Date) {
        ...
        if to < from {
            ...
            clear()
            viewModel.filter(type: filterType, fromDate: from, toDate: to)
